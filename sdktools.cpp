...
int main(int argc, char* argv[])
{
    ...
    void* so_handle = dlopen("/usr/local/lib/libWeWorkFinanceSdk_C.so", RTLD_LAZY);
    if (!so_handle) {
    ...
    ret = init_fn(sdk, "wwbd3253a154312913", "paEgzmciZUFYyxSjC6-UX87Vzm1yMyTCBgN7CPfIx0o");
    if (ret != 0) {
        //sdk需要主动释放
        destroysdk_fn(sdk);
    ...
    if (type == 1) {
        ...
        while (isfinish == 0) {
            ...
            ret = getmediadata_fn(sdk, index.c_str(), argv[2], argv[4], argv[5], timeout, mediaData);
            if (ret != 0) {
            ...
            char file[200];
            snprintf(file, sizeof(file), "%s", argv[3]);
